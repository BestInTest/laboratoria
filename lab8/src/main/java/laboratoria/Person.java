...
//    }
    ...
    public static List<Person> filterByName(List<Person> persons, String subStr) {
        return persons.stream()
                .filter(person -> person.getNazwisko().contains(subStr))
                .collect(Collectors.toList());
    }

    public static List<Person> sortByBirthDate(List<Person> persons) {
        return persons.stream()
                .sorted(Comparator.comparing(Person::getDataUrodzin))
                .collect(Collectors.toList());
    }

    public static List<Person> getSortedByLifespan(List<Person> persons) {
        return persons.stream()
                .filter(person -> person.getDataSmierci() != null)
                .sorted((p1, p2) -> {
                    long lifespan1 = p1.getDataSmierci().toEpochDay() - p1.getDataUrodzin().toEpochDay();
                    long lifespan2 = p2.getDataSmierci().toEpochDay() - p2.getDataUrodzin().toEpochDay();
